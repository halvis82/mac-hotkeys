...
guard AXIsProcessTrusted() else {
    ...
    log("not yet trusted for Accessibility - approve the prompt (or add it in")
    log("System Settings > Privacy & Security > Accessibility), then relaunch.")
    exit(1)
}
...
guard let tap = CGEvent.tapCreate(
    ...
    log("failed to create event tap - grant Input Monitoring to this app in")
    log("System Settings > Privacy & Security > Input Monitoring, then relaunch.")
    exit(1)
}
